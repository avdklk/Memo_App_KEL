...
class ImmediatePanGestureRecognizer: UIGestureRecognizer {
    ...
    var velocity: CGPoint? {
        ...
        let locationP = trackedTouch.location(in: view)
        let delta = CGPoint(x: locationP.x - lastLastPoint.x, y: locationP.y - lastLastPoint.y)
        let deltaT = CGFloat(lastTime - lastLastTime)
        //    return CGPoint(x: delta.x / deltaT , y: delta.y - deltaT) //Q original
        return CGPoint(x: delta.x / deltaT , y: delta.y / deltaT) //gpt
    }
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent) {
        ...
        if startPoint.distance(to: lastPoint) >= tapThreshold {
